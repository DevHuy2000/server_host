...
ADMIN_USERNAME = os.environ.get("ADMIN_USER", "senzu")
ADMIN_PASSWORD = os.environ.get("ADMIN_PASS", "12")

running_procs = {}   # internal_key -> (Popen, log_file_handle)
server_states = {}   # internal_key -> Offline/Installing/Starting/Running/Banned
lock = threading.Lock()
...
def set_state(internal_key: str, state: str):
    with lock:
        server_states[internal_key] = state


def get_state(internal_key: str) -> str:
    with lock:
        return server_states.get(internal_key, "Offline")


# ĐÃ FIX: Tách riêng owner và folder để tránh lỗi session trong background thread
def log_append(owner: str, folder: str, text: str):
    try:
...
def ensure_requirements_installed(owner: str, folder: str):
    ...
    log_append(owner, folder, "[SYSTEM] Installing requirements.txt...\n")
    try:
        ...
        log_append(owner, folder, "[SYSTEM] requirements installed ✅\n")
        return True
    except subprocess.CalledProcessError as e:
        log_append(owner, folder, f"[SYSTEM] requirements install failed: {e}\n")
        return False
...
@app.route("/login")
def login_page():
    return _no_cache(send_from_directory(BASE_DIR, "login.html"))


@app.route("/create")
def create_page():
    return _no_cache(send_from_directory(BASE_DIR, "create.html"))
...
def admin_page():
    if not is_admin_session():
        return redirect("/")
    return _no_cache(send_from_directory(BASE_DIR, "admin.html"))
...
def list_all_servers_for_admin():
    ...
    for owner in sorted(os.listdir(USERS_ROOT)):
        ...
        for folder in sorted(os.listdir(root)):
            ...
            st = "Banned" if banned else get_state(internal_key)
...
def list_servers_for_user(username: str):
    ...
    for folder in sorted(os.listdir(root)):
        ...
        internal_key = f"{username}::{folder}" # ĐÃ FIX: Chuẩn hóa key
        st = "Banned" if banned else get_state(internal_key)
        ...
        servers.append({
            ...
            "folder": folder,
            "owner": username,
            "key": folder,  # user only uses folder to call APIs
...
def server_stats(key):
    ...
    if meta.get("banned", False):
        ...
        state = "Running"
        set_state(internal_key, "Running")
...
def background_start(internal_key: str, owner: str, folder: str, startup_file: str):
    try:
        set_state(internal_key, "Installing")
        log_append(owner, folder, "[SYSTEM] Preparing...\n")

        ensure_requirements_installed(owner, folder)

        set_state(internal_key, "Starting")
        ...
        log_append(owner, folder, f"[SYSTEM] Start failed: {e}\n")
        set_state(internal_key, "Offline")
...
def server_action(key, act):
    ...
    t = threading.Thread(target=background_start, args=(internal_key, owner, folder, startup), daemon=True)
    t.start()
    return jsonify({"success": True})


@app.route("/server/clear-logs/<path:key>", methods=["POST"])
@login_required
def clear_logs_endpoint(key):
    if not can_access_key(key):
        return jsonify({"success": False, "message": "Forbidden"}), 403
...
def admin_server_ban():
    ...
    if banned:
        stop_proc(internal_key)
        set_state(internal_key, "Banned")
        log_append(owner, folder, "[ADMIN] Server banned.\n")
    else:
        set_state(internal_key, "Offline")
        log_append(owner, folder, "[ADMIN] Server unbanned.\n")
...
def admin_server_delete():
    ...
    try:
        ...
        with lock:
            if internal_key in server_states:
                del server_states[internal_key]
